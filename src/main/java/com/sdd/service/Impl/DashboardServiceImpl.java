    ...
    public ApiResponse<DashBoardResponse> getDashBoardData(DashBoardRequest dashBoardRequest) {
        ...
        if (dashBoardRequest.getToUnitId() == null || dashBoardRequest.getToUnitId().isEmpty()) {
            ...
            if (cgFromUnit == null) {
                ...
                budgetList = budgetAllocationDetailsRepository.findByToUnitAndIsDelete(dashBoardRequest.getToUnitId(), "0");
            }
        }

        if (dashBoardRequest.getBudgetFinancialYearId() == null || dashBoardRequest.getBudgetFinancialYearId().isEmpty()) {

        } else {
            BudgetFinancialYear budgetFinancialYear = budgetFinancialYearRepository.findBySerialNo(dashBoardRequest.getBudgetFinancialYearId());
            if (budgetFinancialYear == null) {
                throw new SDDException(HttpStatus.UNAUTHORIZED.value(), "INVALID FINANCIAL YEAR ID");
            } else {
                budgetList = budgetAllocationDetailsRepository.findByToUnitAndFinYearAndIsDelete(dashBoardRequest.getToUnitId(), dashBoardRequest.getBudgetFinancialYearId(), "0");
            }
        }
        ...
        List<BudgetAllocation> budgetAllocation = budgetAllocationRepository.findByToUnitAndIsFlag(hrDataCheck.getUnitId(), "1");


        for (int i = 0; i < budgetAllocation.size(); i++) {

            BudgetHead budgetHeadId = subHeadRepository.findByBudgetCodeId(budgetAllocation.get(i).getSubHead());
            if (budgetHeadId == null) {
                throw new SDDException(HttpStatus.UNAUTHORIZED.value(), "INVALID SUB HEAD TYPE ID.CONTACT YOUR ADMINISTRATOR");
            }
        ...
        for (Map.Entry<String, CgUnit> entry : cgunitData.entrySet()) {
            ...
            List<BudgetAllocation> budgetAllocationList = budgetAllocationRepository.findByToUnitAndFinYearAndIsFlag(unitData.getUnit(), "01", "1");
            double allocationAmount = 0;
            for (int i = 0; i < budgetAllocationList.size(); i++) {
                allocationAmount = allocationAmount + Double.parseDouble(budgetAllocationList.get(i).getAllocationAmount());
            }

            double expenditure = 0;
            List<ContigentBill> cbExpendure = contigentBillRepository.findByCbUnitIdAndIsFlag(unitData.getUnit(), "0");
        ...
        SubHeadWiseExpenditueResponse subHeadWiseExpenditueResponse = new SubHeadWiseExpenditueResponse();
        ...
        for (Map.Entry<String, BudgetHead> entry : subHeadData.entrySet()) {
            ...
            List<BudgetAllocation> budgetAllocationList = budgetAllocationRepository.findByToUnitAndFinYearAndIsFlag(hrDataCheck.getUnit(), "01", "1");
            double allocationAmount = 0;
            for (int i = 0; i < budgetAllocationList.size(); i++) {
                allocationAmount = allocationAmount + Double.parseDouble(budgetAllocationList.get(i).getAllocationAmount());
            }
            ...
            double expenditure = 0;
            List<ContigentBill> cbExpendure = contigentBillRepository.findByBudgetHeadIDAndIsFlag(subHeadD.getBudgetCodeId(), "0");
        ...
        for (int i = 0; i < budgetList.size(); i++) {
            ...
            dashBoardData.setFinancialYearId(budgetFinancialYearRepository.findBySerialNo(budgetList.get(i).getFinYear()));
            dashBoardData.setSubHead(subHeadRepository.findByBudgetCodeIdOrderBySerialNumberAsc(budgetList.get(i).getSubHead()));
...
//        }
    ...
    public ApiResponse<List<AmountUnit>> getAllAmountUnit() {
        ...
        return ResponseUtils.createSuccessResponse(amountUnit, new TypeReference<List<AmountUnit>>() {
        });
    }
    ...
    public HrData getAuthorization(String userInfo) {
        ...
        String userInfoData = "http://172.18.3.150:8080/cghrdata/getAllData/getAllOfficerData/" + userInfo;
        try {
