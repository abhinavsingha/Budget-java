...
public class DashboardServiceImpl implements DashBoardService {
    ...
    public ApiResponse<DashBoardResponse> getDashBoardData(DashBoardRequest dashBoardRequest) {
            ...
            if (budgetFinancialYear == null) {
                ...
                budgetList =
                        budgetAllocationDetailsRepository.findByToUnitAndFinYearAndIsDeleteAndIsBudgetRevision(
                                dashBoardRequest.getToUnitId(), dashBoardRequest.getBudgetFinancialYearId(), "0", "0");
            }
        }
        ...
        List<BudgetAllocation> budgetAllocation =
                budgetAllocationRepository.findByToUnitAndIsFlagAndIsBudgetRevision(hrDataCheck.getUnitId(), "1", "0");
        ...
        for (Map.Entry<String, CgUnit> entry : cgunitData.entrySet()) {
            ...
            List<BudgetAllocation> budgetAllocationList =
                    budgetAllocationRepository.findByToUnitAndFinYearAndIsFlagAndIsBudgetRevision(unitData.getUnit(), "01", "1", "0");
        ...
        try {
            ...
            for (BudgetHead budgetHead : budgetHeadList) {
                ...
                for (Integer i = 0; i < budgetAllocationList.size(); i++) {
                    AmountUnit amountUnit = amountUnitRepository.findByAmountTypeId(budgetAllocationList.get(i).getAmountType());
                    allocationAMount = allocationAMount + (Double.parseDouble(budgetAllocationList.get(i).getAllocationAmount()) * amountUnit.getAmount());
                }
                dashBoardExprnditureResponse.setAllocatedAmount(ConverterUtils.addDecimalPoint(allocationAMount + ""));
