        ...
        for (Integer i = 0; i < budgetAllocationSaveRequestList.getBudgetRequest().size(); i++) {
            ...
            if (!(budgetAllocationSaveRequestList.getBudgetRequest().get(i).getToUnitId().equalsIgnoreCase(hrData.getUnitId()))) {
                ...
                if (actualRivisionAmount > lowestUnitCDARemeingAmount) {
                    throw new SDDException(HttpStatus.UNAUTHORIZED.value(), "REMAINING AMOUNT NOT ENOUGH");
                }
    ...
    public ApiResponse<BudgetAllocationResponse> getAllGroupIdAndUnitIdRevisionCase(String groupId) {
        ...
        for (Integer i = 0; i < budgetAllocations.size(); i++) {
            ...
            for (Integer c = 0; c < subUnitList.size(); c++) {
                ...
                List<ContigentBill> contigentBills = contigentBillRepository.findByCbUnitIdAndFinYearAndBudgetHeadIDAndAllocationTypeIdAndIsUpdateAndIsFlag(hrData.getUnitId(), budgetAllocations.get(i).getFinYear(), budgetAllocations.get(i).getSubHead(), budgetAllocations.get(i).getAllocationTypeId(), "0", "0");
